  ...
  const login = async (username, password) => {
    try {
      ...
      const response = await fetch('https://tfgv2-production.up.railway.app/api/login', {
        ...
        body: JSON.stringify({ 
          usuario: username.trim(), 
          pass: password  // <-- CLAVE: Campo "pass" como espera el backend
        }),
        credentials: 'include'
      });
      ...
      const userData = {
        ...
        polideportivo_id: data.user.polideportivo_id || null
      };
