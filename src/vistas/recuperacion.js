...
export default function RecuperarPassword() {
  ...
  const handleSolicitarCodigo = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setMensajeExito(data.message || 'Si el email existe en nuestro sistema, recibirás un código de verificación');
        setPasoActual(2);
  ...
  const handleReenviarCodigo = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setMensajeExito(data.message || 'Código reenviado correctamente');
